...
const RELEASES = [
  {
    ...
    date: "15 сентября",
    items: [
      "Новый раздел «Тренажёр»: 50 заданий по физике из открытого банка ФИПИ.",
      "Время считает настоящий секундомер — от показа условия до ответа, и он останавливается, когда приложение свёрнуто.",
      "У каждого задания виден его номер в банке, а после ответа — разбор.",
      "Ответы решены нами: банк правильный ответ не отдаёт, поэтому у каждого честно написано, сверен он или нет.",
      "Две кнопки под ответом: подтвердить ключ, если банк согласился, и пожаловаться, если банк ответил иначе.",
    ],
    changes: [
      [
      ...
      [
        ...
        "Ключи решены нами и помечены как несверенные; спорные собираются отдельным списком, чтобы их перепроверить",
      ],
    ],
  },
  {
