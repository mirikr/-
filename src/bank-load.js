...
// Опись для поиска: номер, предмет, раздел и начало условия у каждого задания.
// Она нужна только на экране поиска, поэтому и грузится только там — отдельным
// куском, не трогая ни приложение, ни сами наборы.
let find = null;
export async function loadFind() {
  if (find) return find;
  try {
    find = (await import("./bank-find.js")).FIND || [];
    return find;
  } catch (e) {
    // Без сети и без кеша описи не будет: поиск просто не найдёт заданий,
    // всё остальное искаться продолжит.
    return null;
  }
}
